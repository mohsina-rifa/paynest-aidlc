...
public class TransactionService {
    ...
    public TransactionResponse findResponseById(Integer id) {
        Transaction transaction = findById(id);
        List<String> names = transaction.getCategories().stream()
                .map(Category::getName)
                .toList();
        return new TransactionResponse(transaction.getId(), transaction.getAmount(), names);
    }
